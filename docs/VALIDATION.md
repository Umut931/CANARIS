...
## Matrice PROUVÉ ICI ✅ / À VALIDER EN VM 🖥️

> **Aucun mécanisme 🖥️ n'est présenté comme validé.** Le blocage LSM `-EPERM` et
> le kill réel n'ont pas pu être exécutés ici (Docker/WSL2 : pas de `bpf` LSM
> actif, pas d'init-namespace) — harnais turnkey fournis dans `vm/`.

| # | Prouvé ICI | À valider en VM |
|---|---|---|
| R1 blocage LSM | verifier OK (3 hooks LSM) ; logique | 🖥️ `vm/validate_enforce.sh` |
| R2 kill+préservation | baseline-avant-attaque E2E réel ; kill+latence unit C (47 ms) | 🖥️ `vm/validate_kill.sh` |
| R3 minifilter | revue statique | 🖥️ `vm/build_windows.ps1` |
| R5 VssGuard | matching C+Python | 🖥️ `vm/validate_windows.md` §6 |
| R6/R7 faux positifs | 0 FP synthétique (56 tests) | 🖥️ `vm/fp_workload.sh` (réels) |
| R8 latence | — | 🖥️ `demo/benchmark_io.sh` |
| T1 baseline | ✅ exclut chiffrés, survit réécriture en place, rotation, E2E | — |
| T2 anti-spoof | ✅ exe-inode ; spoofeur `comm=rsync` détecté (E2E réel) | — |
...
## Phase 8 — Durcissement (revue critique T1–T7) — ✅ PROUVÉ ICI

### T1 — Baseline propre AVANT attaque (correction du snapshot réactif inutile)

E2E loader réel : baseline pris avant l'attaque, puis chiffreur de 7 ms.

```
=== baseline PROPRE pris AVANT attaque ? ===  fichiers propres : 30
=== ATTAQUE ===  Simulateur : 30 fichiers « chiffrés » en 7 ms
=== fichiers CHIFFRÉS dans baseline (doit être 0) ===  0
=== contenu original préservé ? ===  donnees utilisateur originales 1
=== log ===  RESTAURATION recommandee depuis le baseline propre : baseline-...
```

Tests : `tests/test_baseline_snapshot.py` (exclusion chiffrés, **survie à la
réécriture en place** — preuve que c'est une copie, pas un lien dur vers la
source —, rotation K, dédup) + `tests/ctest/test_responder.c` (baseline exclut
le `.CANARIS_LOCKED`). La préservation ne dépend PLUS du snapshot réactif.

### T2 — Whitelist par inode d'exécutable, anti-spoof (E2E réel)

```
spoofeur (comm=rsync, binaire /tmp/rsync NON whitelisté)  -> DÉTECTÉ ✓
binaire réellement whitelisté (inode /tmp/rsync)          -> exempté (non détecté)
```

Le `comm` spoofé n'accorde ni exemption ni quota élevé (seuil par défaut
toujours). eBPF lit `task->mm->exe_file` (verifier OK). Tests C + Python anti-spoof.

### T3 — Manifeste hors arbre protégé

Manifeste/liste écrits dans le control-dir (`~/.canaris/` ou `--control-dir`),
`0` fichier manifeste dans la cible — vérifié par test + smoke (`find target -name canary_* = 0`).

### T5–T7 — Harnais VM turnkey (non exécutés ici, c'est l'objet du handoff)

`vm/validate_enforce.sh` (blocage LSM R1), `vm/validate_kill.sh` (kill réel R2),
`vm/build_windows.ps1` + `vm/validate_windows.md` (driver), `vm/fp_workload.sh`
(faux positifs réels) — verdicts PASS/FAIL, refusent PASS hors du bon environnement.
